...
SEPARATOR = "<SEPARATOR>"
FILE_DIRECTORY = "C:\\Users\\MY MSI\\Desktop\\Code\\Socket\\fordown"  # Thư mục chứa các file
    ...
    if os.path.exists(FILE_DIRECTORY):
        for filename in os.listdir(FILE_DIRECTORY):
            ...
            if os.path.isfile(file_path):
                ...
                file_list.append(f"{filename} - {filesize_kb:.1f} KB")
    return file_list
...
def handle_client(client_socket, address):
    ...
    sended_files = []
    # Gửi các file cho client
    while True:
        wanted_files = receive_message(client_socket)
        if wanted_files == "close":
            print("[+]Client address ", address ," closed")
            break
        else:
            wanted_files = wanted_files.split(',')
        for requested_file in wanted_files:
            if requested_file not in sended_files:
                if send_file(client_socket, requested_file.strip()):
                    sended_files.append(requested_file)
                else:
                    continue
            else:
                data = "sended"
